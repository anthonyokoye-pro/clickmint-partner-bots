...
The implemented system is a **three-process Telegram polling application with shared legacy JSON persistence plus additive SQLite repositories, pure Python business rules, an authenticated Admin HTTP/Mini App boundary, and optional Telegram/Telethon integrations**. The Reward Bot owns the Mint exchange/task workflow, the Partnership Bot owns curated contract-based exchange, and the Admin Bot/Mini App provide owner and scoped-admin controls. SQLite currently backs Mint, tasks, broadcasts, credibility, enforcement, backups, and Admin idempotency; the Mint migration boundary is not yet fully authoritative. There is no enabled payment rail, withdrawal system, Boost economy, advertising marketplace, or autonomous AI layer. Any future expansion beyond these boundaries must be documented as a new component rather than assumed to exist.
