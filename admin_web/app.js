(() => {
  ...
  $("create-broadcast").addEventListener("click", async () => { const text = $("broadcast-text").value.trim(); if (!text) return; const scheduled = $("broadcast-scheduled").value ? Math.floor(new Date($("broadcast-scheduled").value).getTime()/1000) : null; try { await api("/api/admin/broadcasts", {method:"POST",headers:{"Content-Type":"application/json","X-Idempotency-Key":crypto.randomUUID()},body:JSON.stringify({text,scheduled_at:scheduled})}); $("broadcast-text").value=""; refresh(); } catch (error) { $("notice").textContent = error.message; } }); $("refresh").addEventListener("click", refresh); $("category").addEventListener("change", refresh); $("broadcast-status").addEventListener("change", refresh); $("audit-type").addEventListener("change", refresh); $("period").addEventListener("change", refresh); refresh();
})();
